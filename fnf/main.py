...
class Note:
    ...
    def update(self):
        # --- MODIFIED: Removed the line that was causing the bug ---
        if not (self.is_hold and self.is_holding): self.y += self.speed; self.rect.centery = int(self.y)
        # if (self.is_hold and self.y - self.full_tail_length > SCREEN_HEIGHT) or (not self.is_hold and self.y > SCREEN_HEIGHT): self.is_active = False # <-- THIS LINE WAS REMOVED
...
class ChartEditor:
    def __init__(self, screen, clock, song_info, sprites):
        ...
        # --- NEW: Variables for selection, copy, and paste ---
        self.highlight_sprites = self._create_highlight_sprites(sprites)
        self.selection_box = None
        self.selection_start_pos = None
        self.selected_notes = set() # A set of note indices
        self.note_clipboard = []
    ...
    # --- NEW: Helper to create highlighted versions of note sprites ---
    ...
    def handle_events(self):
        # --- MODIFIED: Rewrote mouse logic for clarity and to fix the bug ---
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEMOTION:
                if self.selection_start_pos: # If left mouse is down
                    if self.selection_box is None: # Start drawing box if mouse moves
                ...
                if not self.debug_menu_visible:
                    if event.button == 1: # Left click release
                        ...
                        if was_drag:
                            ...
                            # It was a click, not a drag: handle note placement/single select
                            if not (is_shift or is_ctrl):
                                self.selected_notes.clear()
                    ...
                    elif event.button == 3: # Right Click (for removing single notes)
                        mx, my = event.pos
                        lane = self.get_lane_from_mouse(mx)
                        if lane is not None:
                            time_ms = self.get_time_from_mouse(my)
    ...
    # --- NEW: Methods for copy, paste, and delete ---
    ...
    def delete_selection(self):
        ...
        # Rebuild the chart excluding selected indices
    ...
    def draw(self):
        # --- MODIFIED: Draws highlights, selection box, and new UI text ---
        ...
        if self.selection_box:
            ...
            s.fill((100, 100, 255, 60)) # Semi-transparent blue fill
            pygame.draw.rect(s, (150, 150, 255), s.get_rect(), 2) # Border
